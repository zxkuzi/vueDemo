...
EPUBJS.Book.prototype.displayChapter = function (chap, end, deferred) {
    ...
    render = book.renderer.displayChapter(chapter, this.globalLayoutProperties, this.settings.customStyleLinks);
    if (cfi) {
...
EPUBJS.Book.prototype.nextPage = function (defer) {
    ...
    if (!next) {
        if (this.settings.funcEnd && this.settings.funcEnd()) {
            return this.nextChapter(defer);
        } else if (!this.settings.funcEnd) {
            return this.nextChapter(defer);
        }
    }
    defer.resolve(true);
...
EPUBJS.Book.prototype.prevPage = function (defer) {
    ...
    if (!prev) {
        if (this.settings.funcStart && this.settings.funcStart()) {
            return this.prevChapter(defer);
        } else if (!this.settings.funcStart) {
            return this.prevChapter(defer);
        }
    }
...
EPUBJS.Book.prototype.gotoCfi = function (cfiString, defer) {
    ...
    if (!this.isRendered) {
        console.warn("Not yet Re ndered");
...
*/
EPUBJS.Renderer.prototype.displayChapter = function (chapter, globalLayout, customStyleLinks) {
    var store = false;
    if (this._moving) {
    ...
    return chapter.render().
        ...
        if (customStyleLinks && ((customStyleLinks instanceof Array) === true) && customStyleLinks.length > 0) {
            var styleItems = "";
            for (var i = 0; i < customStyleLinks.length; i++) {
                styleItems += '<link rel="stylesheet" type="text/css" href="' + customStyleLinks[i] + '" />';
            }
            contents = contents.replace(/<head>/g, styleItems);
        }
